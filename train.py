...
parser.add_argument("--step_size", type=list, default=[5, 8])  # changed from [120, 180]
...
class BaseTrainer:
    ...
    def set_optimizer(self):
        ...
        # NOTE: This does work decently, but I am going to test and see if updating the mask decoder also helps
    ...
    def set_lr_scheduler(self):
        if self.args.lr_scheduler == "multisteplr":
            ...
            self.lr_scheduler = torch.optim.lr_scheduler.StepLR(
                self.optimizer, int(self.args.step_size[0]), self.args.gamma
            )
    ...
    def train_epoch(self, epoch, num_clicks):
        ...
        epoch_loss /= step
        epoch_dice = self.get_dice_score(
            prev_masks, gt3D
        )  # NOTE: Fixes the dice plot being all 0
...
def main():
    ...
    if args.multi_gpu:
        ...
        random.seed(
            2024
        )  # FIXME: Seed was 2023, now 2024 to see if initialization changes things
        np.random.seed(2024)
        ...
        logging.basicConfig(
            ...
            level=logging.INFO,
            filemode="w",
            filename=os.path.join(LOG_OUT_DIR, f"output.log"),
        )
